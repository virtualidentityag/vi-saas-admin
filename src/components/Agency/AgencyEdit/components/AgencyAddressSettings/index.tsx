...
const AgencyAddressSettingsLocal = ({ form, postCodes }: AgencyAddressSettingsLocalProps) => {
    ...
    return (
        <>
            ...
            {postCodeRangesActive && <PostCodeRanges agencyPostCodeRanges={postCodes} formInputData={form} />}
        </>
    );
};
...
export const AgencyAddressSettings = ({ id }: { id: string }) => {
    ...
    const saveInfo = useCallback(
        (formData) => {
            if (formData.city !== agencyData.city || formData.postcode !== agencyData.postcode) {
                updateAgency({
                    city: formData.city,
