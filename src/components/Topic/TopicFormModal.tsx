...
const TopicFormModal = () => {
    ...
    const onFieldsChange = () => {
        setSubmitButtonDisabled(
            Object.values(formInstance.getFieldsValue(['name', 'description', 'internalIdentifier'])).some(
                (field: any) => field && field.length === 0,
            ) || formInstance.getFieldsError().some((field: any) => field && field.errors.length > 0),
        );
    };
    ...
    return (
        <Modal
            ...
            title={
                <>
                    <Title level={2}>
                        {topicModel.id != null ? t('topic.modal.headline.edit') : t('topic.modal.headline.add')}
                    </Title>
        ...
        >
            ...
            >
                <Item label={t('topic.name')} name="name" rules={[{ required: true, max: 100 }]}>
                    <Input placeholder={t('placeholder.topic.name')} maxLength={100} />
                </Item>
                <Item label={t('topic.description')} name="description" rules={[{ required: true, max: 200 }]}>
                    <TextArea placeholder={t('placeholder.topic.description')} />
                </Item>
                <Item label={t('topic.internalIdentifier')} name="internalIdentifier" rules={[{ max: 50 }]}>
                    <Input placeholder={t('placeholder.topic.internalIdentifier')} maxLength={50} />
                </Item>
