...
const CounselorList = () => {
    ...
    const [editingCounselor, setEditingCounselor] = useState<CounselorData | undefined>(undefined);
    ...
    const updateCounselors = useCallback((counselorData: CounselorData[]) => {
        const modifiedCounselors = counselorData.map((counselor: CounselorData) => ({
            ...counselor,
            ...
            key: counselor.id,
            openStatus: counselor.agencies.length > 1 ? OpenStatus.CLOSED : OpenStatus.NOT_AVAILABLE,
        }));
            ...
            render: (_: any, record: ModifiedCounselorData) => {
                ...
                return (
                    <button
                        ...
                        type="button"
                        onClick={() => updateSingleCounselor(record as ModifiedCounselorData)}
                    >
                        {record.openStatus === OpenStatus.CLOSED && <CustomChevronDownIcon />}
    ...
    const [columnsWidth, setColumnsWidth] = useState(columnsData.map(({ width }) => width));
    ...
    return (
        <>
            ...
            <ModalForm
                title={editingCounselor ? t('counselor.modal.headline.edit') : t('counselor.modal.headline.add')}
                isInAddMode={!editingCounselor}
                isModalCreateVisible={isModalFormVisible}
                handleCreateModalCancel={handleFormModalCancel}
                handleOnAddElement={
                    editingCounselor ? (param) => handleEditCounselor(param, editingCounselor) : handleAddCounselor
                }
                formData={editingCounselor || defaultCounselor}
                renderFormFields={({ form, setButtonDisabled, formData, isInAddMode }: RenderFormProps) => (
                    <Counselor
