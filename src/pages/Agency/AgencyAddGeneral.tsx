...
import { Button, ButtonItem, BUTTON_TYPES } from '../../components/button/Button';
...
export const AgencyAddGeneral = () => {
    const { t } = useTranslation();
    const { isEnabled } = useFeatureContext();
    const [formAgencyAdd] = Form.useForm();
    const [postCodeRangesSwitchActive, setPostCodeRangesSwitchActive] = useState(false);
    const [isTeamAgency, setIsTeamAgency] = useState<boolean>();
    const navigate = useNavigate();
    const [stickyActions, setStickyActions] = useState<boolean>();
    const [stickyActionsPositionBottom, setStickyActionsPositionBottom] = useState<number>(0);
    ...
    const onFinish = () => {
        ...
        formAgencyAdd.validateFields().then((values) => {
            addAgencyData(values).then((id) => {
                ...
                navigate(`${routePathNames.agencyEditGeneral.replace(':id', agencyId.toString())}`);
    ...
    return (
        <div className="addForm">
                ...
                <Row gutter={[20, 10]}>
                    <Col xs={12} lg={6}>
                        <Box>
                            ...
                            <div>
                                <Item label={t('agency.add.general.name')} name="name" rules={[{ required: true }]}>
                                    <Input placeholder={t('agency.add.general.name')} />
                                </Item>
                                <Item label={t('agency.add.general.description')} name="description">
                                    <TextArea placeholder={t('agency.add.general.description')} rows={3} />
                                </Item>
                            </div>
                        </Box>
                        <Box>
                                ...
                                <Row gutter={[20, 10]}>
                                    ...
                                    <Col xs={12} lg={6}>
                                        <Item
                                            ...
                                            rules={[{ required: true }]}
                                        >
                                            <Input placeholder={t('agency.add.general.address.city')} />
                                        </Item>
                                    </Col>
                                </Row>
                                ...
                                >
                                    <div className="flex">
                                        ...
                                        <Paragraph className="desc__toggleText">
                                            {t('agency.add.general.address.zip_code_scope.information')}
                                        </Paragraph>
                                    </div>
                                </Item>
                                {postCodeRangesSwitchActive && (
                                    <PostCodeRanges agencyPostCodeRanges={[]} formInputData={formAgencyAdd} />
                                )}
                            </div>
                        </Box>
                    </Col>
                        ...
                        <Box>
                            ...
                            <div>
                                ...
                                <Row gutter={[20, 10]}>
                                    <Col xs={12} lg={6}>
                                        <Item
                                            label={t('agency.add.general.more_settings.team_advice_center')}
                                            name="teamAgency"
                                        >
                                            <div className="flex">
                                                <Switch
            ...
            <div
                className={`agencyAdd_actions ${stickyActions ? 'agencyAdd_actions--sticky' : ''}`}
