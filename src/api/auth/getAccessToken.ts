...
const getKeycloakAccessToken = (loginProps: {
    ...
    new Promise((resolve, reject) => {
        ...
        const dataBody = `username=${
            tryUnencryptedForEmail ? encodeURIComponent(username) : encodeUsername(username)
        }&password=${encodeURIComponent(password)}${otp ? `&otp=${otp}` : ``}&client_id=app&grant_type=password`;
