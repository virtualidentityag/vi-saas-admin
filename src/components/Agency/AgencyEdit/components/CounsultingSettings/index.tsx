...
function ConsultingSettingsContainer({
  topics,
  hasConsultants,
  ...
  const topicsForList = topics.filter(
    ({ id }) => !topicIds.find(({ value }) => value === `${id}`)
  );
  const gendersForList = Object.values(Gender).filter(
    (name) => !genders.find(({ value }) => value === `${name}`)
  );
...
export function ConsultingSettings({ id }: { id: string }) {
  const [t] = useTranslation();
  const { data: hasConsultants, isLoading: isLoadingConsultants } =
    useAgencyHasConsultants(id);
  ...
  const demographicsInitialValues = isEnabled(FeatureFlag.Demographics)
    ? {
        demographics: {
          ...
          genders: (data?.demographics?.genders || Object.values(Gender)).map(
            (gender) => ({
              value: gender,
              label: t(`agency.gender.option.${gender.toLowerCase()}`),
            })
